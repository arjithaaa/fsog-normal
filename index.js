...
var checkExist = JSON.parse(localStorage.getItem("scores"));
if (checkExist == null) document.querySelector("#best").innerHTML = "BEST TIME : 0.000";
else document.querySelector("#best").innerHTML = "BEST TIME : " + checkExist[0];
...
function editLeaderboard(s) {
  if (checkExist == null) var scoreArray = [];
  else var scoreArray = checkExist;
  ...
  var topScores = checkExist;
  return topScores[0];
}

function viewLdb() {
  document.querySelector(".container").style.zIndex = -1;
  var dispScores = checkExist;
